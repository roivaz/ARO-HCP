...
#   LEASED_ARM_HELPER_SP    — two whitespace-separated ARM helper SP lease names
...
if [[ -n "${LEASED_ARM_HELPER_SP:-}" ]]; then
  ...
  for lease in "${ARM_HELPER_LEASES[@]}"; do
    client_id=$(yq ".armHelperPool.\"${lease}\".clientId" "${ARM_HELPER_POOL_CATALOG}")
    principal_id=$(yq ".armHelperPool.\"${lease}\".principalId" "${ARM_HELPER_POOL_CATALOG}")
    cert_name=$(yq ".armHelperPool.\"${lease}\".certName" "${ARM_HELPER_POOL_CATALOG}")
    if [[ -z "${client_id}" || "${client_id}" == "null" || \
          -z "${principal_id}" || "${principal_id}" == "null" || \
          -z "${cert_name}" || "${cert_name}" == "null" ]]; then
      echo "ERROR: ARM helper lease '${lease}' not found or incomplete in ${ARM_HELPER_POOL_CATALOG}"
      exit 1
    fi
